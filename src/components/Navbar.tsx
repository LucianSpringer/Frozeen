      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20 items-center">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <Link href="/cart" className={`relative p-2 transition-colors ${iconClass}`}>
            ...
            {user ? (
              <div className="relative group">
                ...
                <div className="absolute right-0 w-56 mt-4 origin-top-right bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 divide-y divide-gray-100 dark:divide-slate-700 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform translate-y-2 group-hover:translate-y-0">
                  <div className="p-2">
                    <Link href="/dashboard" className="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 dark:text-slate-200 hover:bg-sky-50 dark:hover:bg-slate-700 hover:text-sky-600 rounded-lg transition">
              ...
              <div className="flex items-center gap-4">
                <Link href="/login" className={`font-medium transition-colors ${iconClass}`}>Masuk</Link>
                <Link href="/register" className="bg-orange-500 text-white px-6 py-2.5 rounded-full font-bold hover:bg-orange-600 transition shadow-lg shadow-orange-500/30 hover:shadow-orange-500/50 transform hover:-translate-y-0.5">
                  Daftar Reseller
                </Link>
              </div>
            )}
          ...
          <div className="flex md:hidden items-center gap-4">
            ...
            <Link href="/cart" className={`relative p-2 ${isHome && !isScrolled ? 'text-white' : 'text-slate-600 dark:text-slate-300'}`}>
              <ShoppingCart size={24} />
              {cartCount > 0 && (
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 shadow-xl absolute w-full left-0 top-20">
          <div className="px-4 pt-4 pb-6 space-y-2">
            <Link href="/" onClick={() => setIsMenuOpen(false)} className={`block px-4 py-3 rounded-lg text-base font-medium ${pathname === '/' ? 'text-sky-600 bg-sky-50 dark:bg-slate-800' : 'text-gray-700 dark:text-slate-200 hover:text-sky-600 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>Beranda</Link>
            <Link href="/products" onClick={() => setIsMenuOpen(false)} className={`block px-4 py-3 rounded-lg text-base font-medium ${pathname === '/products' ? 'text-sky-600 bg-sky-50 dark:bg-slate-800' : 'text-gray-700 dark:text-slate-200 hover:text-sky-600 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>Katalog Produk</Link>

            {user ? (
              <>
                <Link href="/dashboard" onClick={() => setIsMenuOpen(false)} className={`block px-4 py-3 rounded-lg text-base font-medium ${pathname === '/dashboard' ? 'text-sky-600 bg-sky-50 dark:bg-slate-800' : 'text-gray-700 dark:text-slate-200 hover:text-sky-600 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>Dashboard</Link>
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                <Link href="/login" onClick={() => setIsMenuOpen(false)} className="block text-center px-4 py-3 rounded-lg text-base font-medium border border-slate-200 dark:border-slate-700 text-gray-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800">Masuk</Link>
                <Link href="/register" onClick={() => setIsMenuOpen(false)} className="block text-center px-4 py-3 rounded-lg text-base font-medium bg-orange-500 text-white hover:bg-orange-600">Daftar</Link>
              </div>
            )}
          </div>
        </div>
