...
const Dashboard: React.FC = () => {
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-24 min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors">
        ...
        <div className="flex border-b border-slate-200 dark:border-slate-700 mb-6">
          ...
          <button onClick={() => setActiveTab('users')} className={`px-4 py-2 font-medium transition-colors ${activeTab === 'users' ? 'border-b-2 border-sky-600 text-sky-600 dark:text-sky-400' : 'text-slate-500 dark:text-slate-400 hover:text-sky-500'}`}>User & Reseller</button>
        </div>
        ...
        {activeTab === 'users' && (
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm overflow-hidden animate-fade-in transition-colors">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {users.map(u => (
                  <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 text-slate-700 dark:text-slate-300">
                    <td className="p-4 text-sm font-bold">{u.name}</td>
                    <td className="p-4 text-sm">{u.email}</td>
                    <td className="p-4 text-sm capitalize">{u.role}</td>
                    <td className="p-4">
                      <span className={`px-2 py-1 text-xs font-bold rounded-full ${u.status === 'active' ? 'bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300' : u.status === 'pending' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/50 dark:text-yellow-300' : 'bg-red-100 text-red-700'}`}>
                        {u.status?.toUpperCase() || 'ACTIVE'}
                      </span>
                    </td>
                    <td className="p-4">
                      {u.status === 'pending' && u.role === 'reseller' && (
                        <button
                          onClick={() => approveReseller(u.id)}
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-24 min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors">
      ...
      {user.role === 'reseller' && user.status === 'pending' && (
        <div className="bg-yellow-50 dark:bg-yellow-900/20 border-l-4 border-yellow-400 p-4 mb-8 rounded-r-lg flex items-start gap-3">
          ...
          <div>
            <h3 className="font-bold text-yellow-700 dark:text-yellow-400">Akun Menunggu Persetujuan</h3>
            <p className="text-sm text-yellow-600 dark:text-yellow-300">Anda belum bisa mendapatkan harga khusus reseller sampai admin menyetujui pendaftaran Anda. Hubungi admin via WhatsApp untuk percepatan.</p>
          </div>
        </div>
      )}
      ...
      {user.role === 'reseller' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 animate-fade-in-up">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700">
            ...
            <p className="text-3xl font-bold text-slate-900 dark:text-white">{users.filter(u => u.uplineId === user.id).length} <span className="text-sm font-normal text-slate-500 dark:text-slate-400">member</span></p>
          </div>
        </div>
      )}
                ...
                <tr key={order.id} className="text-slate-700 dark:text-slate-300">
                  ...
                  <td className="p-4">
                    ...
                    {order.status === 'pending' && order.paymentMethod === 'transfer' && (
                      <button
                        onClick={() => alert('Fitur upload bukti transfer akan segera hadir!')}
                        className="ml-2 text-xs bg-sky-100 text-sky-600 px-2 py-1 rounded hover:bg-sky-200 transition flex items-center gap-1 inline-flex"
                      >
