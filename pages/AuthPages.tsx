...
export const RegisterPage: React.FC = () => {
  ...
  const navigate = useNavigate();
  const [formData, setFormData] = useState({ name: '', email: '', role: 'reseller' as UserRole, referralCode: '' });
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    const result = register(formData.name, formData.email, formData.role, formData.referralCode);
    if (result.success) {
      navigate('/dashboard');
    } else {
...

          {
            formData.role === 'reseller' && (
              <div className="animate-fade-in">
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Kode Referral (Opsional)</label>
                <input
                  className="w-full border dark:border-slate-600 rounded-lg px-4 py-2 focus:ring-2 focus:ring-sky-500 outline-none bg-white dark:bg-slate-900 text-slate-900 dark:text-white"
